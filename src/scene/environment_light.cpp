...
namespace CGL { namespace SceneObjects {
  ...
  void EnvironmentLight::init() {
    ...
    for(int row = 0; row < h; row++)
      for(int col = 0; col < w; col++)
        pdf_envmap[row * w + col] /= sum;

    for(int row = 0; row < h; row++)
      for(int col = 0; col < w; col++)
        marginal_y[row] += pdf_envmap[row * w + col];

    for(int row = 0; row < h; row++)
      for(int col = 0; col < w; col++)
        conds_y[row*w+col] = pdf_envmap[row*w+col] / marginal_y[row];

    for(int row = 1; row < h; row++)
      marginal_y[row] += marginal_y[row-1];
    
    for(int row = 0; row < h; row++)
      for(int col = 1; col < w; col++)
        conds_y[row * w + col] += conds_y[row * w + col - 1];
  ...
  double EnvironmentLight::sample_L(const Vector3D p, Vector3D* wi,
    ...
    uint32_t h = envMap->h, w = envMap->w;
    uint32_t x,y;
    Vector2D important,angle_coor,uniform = sampler_uniform2d.get_sample();
    y = std::upper_bound(marginal_y,marginal_y + h,uniform.y) - marginal_y;
    x = std::upper_bound(conds_y + y * w, conds_y + (y + 1) * w,uniform.x) - (conds_y + y * w);
    important.x = x;
    important.y = y;
    angle_coor = xy_to_theta_phi(important);

    *wi = theta_phi_to_dir(angle_coor);
    *distToLight = INF_D;
  ...
  Vector3D EnvironmentLight::sample_dir(const Ray& r) const {
    ...
    Vector3D ret;
    ret = bilerp(theta_phi_to_xy(dir_to_theta_phi(r.d)));
    return ret;
